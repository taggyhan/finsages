document.addEventListener('DOMContentLoaded', function () {
  ...
  const updateButton = document.querySelector(
    '#update-transaction-form button[type="button"]',
  );
  if (updateButton) {
...
function validateAmountInput() {
  this.value = this.value
    .replace(/[^0-9\.]/g, '') // Remove any character that is not a number or decimal point
  ...
  if (!this.value.match(/^\d*\.?\d{0,2}$/)) {
    amountError.textContent =
      'Please enter a valid amount (numbers only, max two decimals).';
  } else {
...
function setupPagination(tableId, rowsPerPage) {
  ...
  window[`paginate${tableId}`] = function (direction) {
    ...
    if (direction === 'next' && currentPage < pageCount) {
      ...
      errorDiv.textContent = `You are at the ${
  ...
  fetch('../update-transaction/', {
    ...
    .then((data) => {
      if (data.success) {
        ...
        row.querySelector('td.date').textContent = formatDateForDisplay(
          transaction.date,
        );
        row.querySelector(
          'td.amount',
        ).textContent = `${currency_symbol}${transaction.amount}`;
...
function showPopup(message) {
  ...
  popup.className =
    'fixed bottom-0 right-0 mb-4 mr-4 px-4 py-2 bg-green-500 text-white rounded';
