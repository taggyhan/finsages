...
function openUpdateForm(transactionId) {
  ...
  const amount = row
    .querySelector(`#amount-${transactionId}`)
    .textContent.trim()
    .replace(currency_symbol, '');
  const description = row
    .querySelector(`#description-${transactionId}`)
    .textContent.trim();
  ...
  fetch("{% url 'update_transaction' %}", {
    ...
    .then((data) => {
      if (data.success) {
        ...
        document.getElementById(`date-${transaction.id}`).textContent =
          transaction.date;
        document.getElementById(
          `amount-${transaction.id}`,
        ).textContent = `${currency_symbol}${transaction.amount}`;
        document.getElementById(`description-${transaction.id}`).textContent =
