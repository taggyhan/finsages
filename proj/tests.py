...
class UserModelTest(TestCase):
    def test_user_time_zone_set_based_on_country(self):
        user = User.objects.create_user(
            username="testuser", password="12345", country="IN"
        )
        user.save()
...
class AssetModelTest(TestCase):
    ...
    def test_asset_creation(self):
        ...
        self.assertEqual(
            str(asset), f"Stock owned by {self.user.username} valued at 10000.0"
        )
...
class TransactionModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="transactionuser", password="12345"
        )
    ...
    def test_transaction_creation(self):
        ...
        self.assertEqual(
            str(transaction),
            f"inflow transaction of 100.0 in category 'food' on {transaction.date}",
        )
...
class ViewsTestCase(TestCase):
    ...
    def test_delete_transaction_view(self):
        response = self.client.post(
            reverse("delete_transaction", args=[self.transaction1.id])
        )
        # Check for successful response
        self.assertEqual(response.status_code, 200)
        # Check for expected JSON response content
    ...
    def test_update_goal_amount_view(self):
        response = self.client.post(
            reverse("update_goal_amount", args=[self.goal.id]), {"amount": 200.00}
        )
